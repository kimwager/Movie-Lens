...
The performance of the regularized combined model (RMSE 0.8652) is only slightly better than the combined model. It may be that there is a genre effect, so we can calculate regularized genre effects and combine it with the regularized movie and user effects.
...
The regularized model with genre effects has negligible effect on the performance with an RMSE of 0.8649. This means that the movie effects may already implicitly capture the genre effects. I also note that the genres are grouped, which means that the effects of a single genre may be diluted. My next step was to split out individual genres to run a more specific genre effects model; however my computer did not have enough memeory to compute the task, even on the validation set. 

The only parameters that remain untested are timestamp and year. Niether of these improved the baseline RMSE of 1.06, so will not be included in the final model.
...
Now I will test the final model, which includes regularized movie, user and genre effects on the holdout dataset.
...
From the boxplot, we can determine:

Prediction Bias:


The model appears to compress predictions toward the middle ratings (2.5-3.5)
It tends to overpredict low ratings (0.5-2.5) and underpredict high ratings (4-5)
This is visible in how the median predicted ratings (blue box centers) don't align with the actual ratings (x-axis)


Prediction Spread:


The boxes (showing the interquartile range) get wider for middle ratings
This suggests more uncertainty in predictions for middle ratings
Predictions are more consistent (narrower boxes) for extreme ratings (very low or very high)


Outliers:


Many outlier predictions (black dots) span the full range of possible ratings (0-6)
This indicates the model sometimes makes significant errors
More outliers appear in the middle ratings, suggesting these are harder to predict accurately


Model Limitations:


The diagonal pattern isn't perfect (an ideal model would have box centers perfectly aligned with actual ratings)
This suggests the model has difficulty capturing the full range of rating behaviors
The compression toward middle values is a common issue in recommendation systems (regression to the mean)