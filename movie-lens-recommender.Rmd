...
-   A simple implementation to verify our RMSE calculation function
-   A benchmark that any more complex model should beat

## Step 1: Calculate the mean rating (μ)

This calculates μ (mu), which represents the average of all ratings in our training dataset (edx). We use this as our simplest possible prediction - assuming every movie would be rated at this average value.
...
This creates a vector of predictions by repeating the mean rating (mu) for each row in our test dataset. The rep() function repeats the value, and nrow(final_holdout_test) tells it how many times to repeat.
...
The RMSE function calculates the Root Mean Square Error, which measures how far our predictions deviate from the actual ratings. It works by:
