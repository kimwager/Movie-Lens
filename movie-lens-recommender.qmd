...
#| fig-cap: "RMSE vs Regularization Parameter (λ)"
#| fig-subcap: 
#|   - "Regularized movie and user effects"
#|   - "Regularized movie, user and genre effects"
#| layout-ncol: 2

# Load required package for combining plots
library(patchwork)

# Display the saved plots side by side
initial_reg_plot | final_reg_plot

```

The plots show that:

-   For the initial model, the optimal λ value was `r round(movie_model$optimal_lamda, 2)`

-   For the final model with all effects, the optimal λ value was `r round(optimal_lamda, 2)`
