...
-   Movie Effects Model RMSE: 

-   User Effects Model RMSE: 

-   Combined Effects Model RMSE: 

-   Regularized Model RMSE: 

-   Genre Effects Model RMSE: 

-   Genre-Specific User Effects RMSE: 
...
The regularization parameter (λ) was tuned to find the optimal balance between fitting the training data and preventing overfitting. The plots below show how the RMSE changes with different values of λ for both the initial model and the final model with all effects.

![](initial_reg_plot.png){fig-align="left" width="414"} ![](final_reg_plot.png){fig-align="right" width="414"}

The plots show that:

-   For the initial model, the optimal λ value was `r round(movie_model$optimal_lambda, 2)`

-   For the final model with all effects, the optimal λ value was `r round(optimal_lambda, 2)`
...
-   RMSE on final holdout test set: 
...
-   The final model achieved an RMSE of `r round(final_rmse, 4)` on the holdout test set
