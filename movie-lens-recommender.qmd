...
Where: μ is global mean, b_i is movie-specific effect, b_u is user-specific effect, λ is the regularization parameter.
...
This model extends the regularized model by adding genre-specific effects, recognizing that certain genres may be systematically rated differently. To implement this model, `b_g` was regularized using the λ found optimal for movie and user effects and applied by adding λ to the denominator of the regularization formula. Missing values were handled as described previously. The final prediction formula was:

$rating = μ + b_i + b_u + b_g$
